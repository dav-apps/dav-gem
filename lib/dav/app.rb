module Dav
   class App
      ...
      def get_active_users(jwt, start_timestamp = Time.now.to_i - 2629746, end_timestamp = Time.now.to_i)
         url = $api_url + "apps/app/#{@id}/active_users?start=#{start_timestamp}&end=#{end_timestamp}"
         result = send_http_request(url, "GET", {"Authorization" => jwt}, nil)
         if result["code"] == 200
